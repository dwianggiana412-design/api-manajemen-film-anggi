require('dotenv').config();
const sqlite3 = require('sqlite3').verbose();
const DB_SOURCE = process.env.DB_SOURCE;

const db = new sqlite3.Database(DB_SOURCE, (err) => {
    if (err) {
        console.error(err.message);
        throw err;
    } else {
        console.log('Terhubung ke basis data SQLite.');

        // 1. Skema MOVIE
        db.run(`CREATE TABLE IF NOT EXISTS movies (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            title TEXT NOT NULL,
            director TEXT NOT NULL,
            year INTEGER NOT NULL
            -- Catatan: Jika Anda menggunakan POST /movies yang dilindungi di server.js,
            -- pastikan skema movies di sini sesuai dengan kolom: title, description, release_year, user_id
        )`, (err) => {
            if (!err) {
                const insert = 'INSERT INTO movies (title, director, year) VALUES (?,?,?)';
                // Hindari insert duplikat jika tabel sudah ada, ini hanya seed awal.
                // db.run(insert, ["Parasite", "Bong Joon-ho", 2019]);
                // db.run(insert, ["The Dark Knight", "Christopher Nolan", 2008]);
            }
        });

        // 2. Skema DIRECTORS - PERBAIKAN DI SINI
        db.run(`CREATE TABLE IF NOT EXISTS directors (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            name TEXT NOT NULL UNIQUE,
            birthYear INTEGER 
            -- birthYear dihapus dari NOT NULL, jadi opsional. name diberi UNIQUE
        )`, (err) => {
            if (!err) {
                const insert = 'INSERT INTO directors (name, birthYear) VALUES (?,?)';
                // db.run(insert, ["Bong Joon-ho", 1969]);
                // db.run(insert, ["Christopher Nolan", 1970]);
            }
        });

        // 3. Skema USERS
        db.run(
            `CREATE TABLE IF NOT EXISTS users (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                username TEXT NOT NULL UNIQUE,
                password TEXT NOT NULL,
                role TEXT NOT NULL DEFAULT 'user'
            )`, (err) => {
                if (err) {
                    console.error("Gagal membuat tabel users:", err.message);
                }
            }
        );
    }
});

module.exports = db;